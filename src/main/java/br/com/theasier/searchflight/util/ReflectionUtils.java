...
public class ReflectionUtils extends org.springframework.util.ReflectionUtils {
	...
	public static Field getField(Class<?> classe, String fieldName) {
		while (classe != null) {
			...
			if (fields != null && fields.length > 0) {
				for (Field f : fields) {
					if ( f.getName().equals(fieldName) || f.getName().toLowerCase().equals(fieldName.toLowerCase()))
						return f;
				}
			}
	...
	public static <T> T instantiateClass(Class<T> classe) {
		try {
			...
			throw new RuntimeException("Não foi possível criar uma nova instância da classe '" + classe.getName() + "'.", e);
		}
	}
	...
	public static boolean hasGetterSetter(GenericEntitySearch obj, String nomeAtributo) {
		Method getter = getMethod(obj.getClass(), "get" + org.apache.commons.lang3.StringUtils.capitalize(nomeAtributo));
		Method setter = getMethod(obj.getClass(), "set" + org.apache.commons.lang3.StringUtils.capitalize(nomeAtributo));
